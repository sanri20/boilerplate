...
const rawPackageJSONTemplate = readFileSync(join(ROOT_PATH, 'templates', 'package.hbs.json'), { encoding: 'utf8' })
...
packages.forEach(({ packageName, packageJSONPath, packageJSON, expectedPackageJSON }) => {
  describe(`${packageName}`, () => {
    // eslint-disable-next-line jest/valid-title
    describe(packageJSONPath, () => {
      it('specifies ThingsFactory as author', () => {
        expect(packageJSON.author).toBe(expectedPackageJSON.author)
      })

      it('specifies ThingsFactory Issues as bugs URL', () => {
      ...
      it('specifies ThingsFactory deep-link homepage', () => {
      ...
      it('specifies ThingsFactory‘s public publishConfig', () => {
        expect(packageJSON.publishConfig).toStrictEqual(expectedPackageJSON.publishConfig)
      })

      it('specifies a repository deep-linking into the ThingsFactory monorepo', () => {
    ...
    it(`ensures packages includes in dependencies are used`, () => {
      ...
      }).filter(dep => IGNORE_PACKAGES.every(ignorePackage => !dep.includes(ignorePackage)))
      ...
      const expectValue = dependencies.filter(dep => filesContent.some(content => content.includes(dep)))
...
function readPackages() {
  return glob.sync(join(packagesPath, '*', 'package.json')).map(absolutePackageJSONPath => {
    const packageName = dirname(relative(packagesPath, absolutePackageJSONPath))
